...
export default function Analytics({ client }) {
  ...
  const stats = useMemo(() => {
    const done = videos.filter(v => (v[keys.status] || 'not_started') === 'finished');
    const totalSec = videos.reduce((a, v) => a + (v[keys.totalSeconds] || 0), 0);
    const totalIdleSec = videos.reduce((a, v) => a + (v[keys.idleGaps] || []).reduce((s, g) => s + g, 0), 0);
    const activeWorkSec = Math.max(0, totalSec - totalIdleSec);
    const revenue = done.reduce((a, v) => a + (v.price || 0), 0);
    const totalPotential = videos.reduce((a, v) => a + (v.price || 0), 0);
    const avgTime = done.length ? Math.round(totalSec / done.length) : 0;
    const overdue = videos.filter(v => v.deadline && new Date(v.deadline) < new Date() && (v[keys.status] || 'not_started') !== 'finished').length;
    return { done: done.length, total: videos.length, totalSec, totalIdleSec, activeWorkSec, revenue, totalPotential, avgTime, overdue };
  }, [videos, keys]);
  ...
  // Cumulative progress data for area chart
  ...
  return (
    <div style={{
      ...
      {videos.some(v => (v[keys.finishedCount] || 0) > 0) && (
        <ChartPanel title="Task Completion Frequency" emoji="🔁" height="auto">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {videos.filter(v => (v[keys.finishedCount] || 0) > 0).map((v, i) => {
